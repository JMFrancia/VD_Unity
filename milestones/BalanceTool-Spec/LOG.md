...
**Built:**
- `Agent/Session.cs` — the session directory contract (`sessions/<date>-<slug>/` with `goal.json`, `config.start.json`, `config.current.json`, `journal.jsonl`, `report.md`), `IterationRecord`, `Start`/`Resolve`/`AppendIteration`/`ReadJournal`, and **`GenerateReport` built purely from the journal + a JSON deep-diff of start↔current** (never narrated). Diff labels array elements by their id (`Recipes[field.wheatGrow].Duration`), not index.
- `Cli/Program.cs` — `session start | status | report` sub-verbs (two-word verb, parses its own opts). **`eval --session <name> --rationale <why> [--path P --value V | --patch F]` is the iteration primitive:** it patches `config.current.json` in place, persists it, sims, scores, and appends ONE journal line; `--rationale` required. Non-session `eval` still appends to the flat `runs.jsonl` (unchanged).
- `Api/Server.cs` — `GET /api/sessions`, `GET /api/session?name=X` (returns goal + config.current + journal[]). The browser re-sims `current` through the existing `/api/sim`; no new economy path.
- `wwwroot/app.js` — a third header mode **Session**: polls the active session every 2 s, draws the loss curve (from `journal.jsonl`), an iteration log (patch + rationale), and — re-simming `config.current` at a 5-seed preview — the M06 time-per-level + pressure heatmap. Reuses the M06 chart builders unchanged.
- `README.md` (ledger semantics gross-of-relief, optimality dial + gem row, asset round trip, session workflow), `AGENTS.md` (Balancing sessions section), `.claude/skills/balance_game/SKILL.md` (the 4-step workflow, autonomy boundary, infeasibility stopping rule, "report generated not narrated").
- **Spec amendment:** `docs/VoidDay-Spec-unity.md` §9 + §16 — inspector stays the authoring surface + runtime source of truth; an offline tool reads/writes the assets; one-way dependency; the *game* still has no write endpoint. Docs-only.
- `SessionTests.cs` (4): report traces to journal exactly (30 iters), report contains only journalled iterations, journal round-trips rationale+patch, DiffConfigs finds exactly the changed scalar. **33/33 pass** (29 inherited + 4).

**Verified** (`dotnet test` + built dll + live server via curl; no Unity; **no live browser — Chrome extension not connected**):
- Full acceptance replay **QA-1 … QA-26** (all 26 per the run gate, incl. gem QA-22–26). Live PASS: QA-1,2,3,4,5,6,7,9,11,13,14,15,16,18,19,20,21,22,26. **Documented caveats** (sim reality, not tool bugs — all consistent with M03/M05/M06 LOGs): QA-8/QA-10 (baseline is storage-/unlock-**inert** — self-sustaining pre-placed corn field; mechanisms proven by unit tests), QA-12 (single-config loss non-monotonic from the order-stream shift; monotonic descent proven by `GoalLossIsMonotonic` over a range), QA-17 (baseline buildCost-999999 won't stall — corn loop; stall/cycle guards proven by unit tests), QA-23/24/25 (the conservative CLI player spends 0 gems in reachable configs, so the relief/waste/pending-skip paths aren't live-exercised; the gross-of-relief invariant + `SkipCostMatchesCoreRule` + pending-remedy suppression are proven by M03 unit tests).
- QA-19 in detail: a real 25-iteration session → `session report` → **6/6 trace checks** (iteration count truthful, iter1+iter12 rationales verbatim, last loss present, a journalled patch path present, no phantom iteration 26).
- QA-20 in detail: `goal.json` content == agreed goal; **decline** (no `--apply`) leaves `Assets/` empty; **approve** (`--apply`) writes exactly the 4 summarised scalar changes as one-line diffs; reverted clean.
- CLI vs `/api/sim` determinism identical to the decimal (`3244.371341`, L20). `node --check` clean on `app.js`.

**Deviations from the plan:** none of substance.
- The plan lists only `session start/status/report` as new verbs; the **iteration is recorded through the existing `eval` verb made session-aware** (`--session` + `--rationale`), rather than inventing a fourth verb. This folds patch→persist→sim→journal into one atomic step, which is what keeps a patch from landing un-journalled. `patch --session` was deliberately NOT added (would create a patch-without-rationale gap).
- The plan's "run QA-1 … QA-21" was superseded at the run gate: the full suite through **QA-26** was run (the gem cases QA-22–26 post-date that line).

**Tech debt:**
- The report's export diff is a generic JSON deep-diff of start↔current (truthful, id-labelled) rather than the M02 writer's own scalar plan; the two agree on scalars but the diff also surfaces nested changes the writer would *refuse* on push. Honest, but a reader should know the diff shows "what changed", not "what will push cleanly".
- `write --config` still takes a **file path**, not a version name (M02 contract), whereas `sim`/`eval`/`patch --config` accept a version name or path. Not reconciled — out of M07 scope. In the session flow, push the session's `config.current.json` by path.
- The live Session view re-sims `config.current` at 5 seeds on every new iteration; fine for localhost, not optimised. Not visually confirmed in a browser (extension offline) — data + `node --check` verified.

**Gotchas for later milestones:**
- **★ Session journal schema (any future tool-user inherits):** `journal.jsonl` line = `{Iteration, Ts, Patch[], ConfigHash, Loss, Breakdown, Rationale}`, PascalCase keys (Newtonsoft default, matching `runs.jsonl` + the whole app). `Rationale` is required and is the only field the report can't regenerate. The report is generated from these lines + `config.start`/`config.current` — **never narrate a report**.
- **★ The iteration primitive is `eval --session … --rationale …`, not a new verb.** `suggest`/`sweep` journal nothing (free exploration); only a committed `eval --session` lands a line. Don't "helpfully" make `sweep` journal.
- **★ The gem live-demo gap is real and inherited:** the M03 player is conservative and won't spend gems in reachable CLI configs, so QA-23/24/25 can't be shown end-to-end from the CLI — they're locked by M03 unit tests. If a future milestone needs a live gem-spend demo, it must engineer a config that forces the player to skip (long timers the player is *blocked* on with gems in hand), not baseline.
- `sessions/` is intentionally **not** gitignored (spec) but throwaway runtime sessions are **not** committed — the tool `mkdir`s `sessions/` on demand. `goals/` likewise is a user-input dir created per run, not a committed seed (only `versions/baseline.json` is a required seed).
